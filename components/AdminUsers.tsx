...
import { Users, Search, Building2, User, RefreshCw, Mail, Phone, BadgeCheck, XCircle, AlertCircle } from 'lucide-react';
...
const AdminUsers: React.FC<AdminUsersProps> = ({ token }) => {
  ...
  const fetchUsers = async () => {
    ...
    try {
      ...
      // Merge data (Super Admin + DB Users)
      // The API now only returns Admins
      setUsers([superAdmin, ...data]);
    } catch (error) {
      console.error("Failed to fetch users", error);
      setError("Failed to load user list. Please check your connection.");
      // Fallback: Show Super Admin so list isn't empty
      setUsers([superAdmin]);
    } finally {
      setLoading(false);
    }
